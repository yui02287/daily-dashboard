...
/* ---------- 設定（使用者需填入） ---------- */
const CONFIG = {
  OPENWEATHER_API_KEY: 'YOUR_OPENWEATHER_API_KEY',  // 填入 OpenWeatherMap API Key
  GOOGLE_CLIENT_ID:    'YOUR_GOOGLE_CLIENT_ID',      // 填入 Google OAuth Client ID
  WEATHER_LANG:        'zh_tw',
...
class CalendarClient {
  ...
  init() {
    if (!this.isReady() || CONFIG.GOOGLE_CLIENT_ID === 'YOUR_GOOGLE_CLIENT_ID') return;
...
class WeatherWidget {
  ...
  async init() {
    ...
    if (CONFIG.OPENWEATHER_API_KEY === 'YOUR_OPENWEATHER_API_KEY') {
      this.renderError('請在 js/app.js 設定 OPENWEATHER_API_KEY');
      return;
    }
  ...
  _attachListeners() {
    ...
    document.getElementById('gcal-btn')?.addEventListener('click', async () => {
      if (CONFIG.GOOGLE_CLIENT_ID === 'YOUR_GOOGLE_CLIENT_ID') {
        this.renderer.showToast('請先設定 GOOGLE_CLIENT_ID', 'error');
        return;
      }
      if (this.calClient.isConnected()) {
